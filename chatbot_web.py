...
document_analysis_prompt = PromptTemplate.from_template(
    ...
    2. 문서에 없는 내용은 답변하지 말 것
    3. 관련 정책이 없다면 "관련 정책 정보 없음"이라고 답
    4. 추출된 정보를 바탕 최대 10가지 공약을 출력하며 추출된 정보의 원문은 그대로 표기할 것
    5. 추출한 정책 수가 10개 미만일 경우 추출된 정책 수 만큼 출력하고 추가로 생성하지는 말 것
    6. 각 정책은 핵심 내용만 간결하게 30자 내로 작성
    7. 각 정책은 반드시 번호를 붙여 구분하고(1. 2. 3. 등), 정책별로 한 줄씩 띄워주세요.
