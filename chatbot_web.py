...
document_analysis_prompt = PromptTemplate.from_template(
    ...
    # 중요 지시사항
    - 만약 질문이 이전에 언급된 특정 번호 항목(예: "3번", "4번 항목", "5번")에 대한 상세 설명을 요청하는 경우,
      다른 항목을 모두 무시하고 해당 항목만 심층적으로 분석하여 자세히 설명해주세요.
    - 항목 설명 시 최소 50단어 이상의 충분한 상세 정보를 제공하세요.
    - 내용이 불충분할 경우 정책의 배경, 목적, 기대효과, 유사 해외사례 등을 추가하여 설명을 풍부하게 해주세요.
    - 항목 설명 요청일 경우, 일반적인 목록 형식이 아닌 단락 형태의 상세한 설명으로 답변해주세요.
...
def format_response(question, analyzed_info):
    ...
    # 중복된 항목 패턴 제거 (번호가 다시 시작되는 패턴 제거)
    pattern = r'1\.\s+\*\*.*?\*\*:'
    if re.search(pattern, final_answer[100:]):  # 처음 100자 이후에 1번 항목이 다시 나타나면
        # 첫 번째 1번 항목 이후의 텍스트만 유지
        first_item_match = re.search(pattern, final_answer)
        if first_item_match:
            first_item_pos = first_item_match.start()
            second_item_match = re.search(pattern, final_answer[first_item_pos+100:])
            if second_item_match:
                second_item_pos = first_item_pos + 100 + second_item_match.start()
                final_answer = final_answer[:second_item_pos]
