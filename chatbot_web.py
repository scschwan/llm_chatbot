...
document_analysis_prompt = PromptTemplate.from_template(
    ...
    5. 관련 정책이 없다면 "관련 정책 정보 없음"이라고만 답하세요.
...
먼저 소스 코드를 분석해보니, 현재 chatbot_web.py는 FastAPI 기반의 웹 서버를 구현하고 있고, 채팅 API 엔드포인트에서 메시지를 처리하는 방식은 한 번에 전체 응답을 반환하는 방식으로 구현되어 있습니다. 스트리밍 방식으로 변경하기 위해 다음과 같이 수정해야 합니다.

스트리밍 방식으로 수정하는 방법
FastAPI의 스트리밍 응답을 사용하기 위해 먼저 필요한 클래스를 임포트해야 합니다.
LLM 파이프라인에서 스트리밍을 활성화해야 합니다.
API 엔드포인트를 스트리밍 응답을 반환하도록 수정해야 합니다.
다음은 수정 방법입니다:

1. 필요한 임포트 추가
python
from fastapi.responses import StreamingResponse
파일 상단의 임포트 섹션에 위 라인을 추가합니다.

2. LLM 파이프라인에서 스트리밍 활성화
init_rag_system 함수에서 HuggingFace 파이프라인 생성 부분을 수정합니다:
...
3. 스트리밍 함수 구현
스트리밍 응답을 처리하기 위한 새로운 함수를 추가합니다:
...
4. 채팅 API 엔드포인트 수정
이제 채팅 API 엔드포인트를 스트리밍 응답을 반환하도록 수정합니다:
